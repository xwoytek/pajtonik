...
class runyy:
    def __init__(self,runaognia:int,runawody:int,runaziemi:int,runapowietrza:int,runaataku:int,runadefensu:int,runahp:int) -> None:
...
class sklep:
    ...
    def rundawsklepie(self):
        ...
        while czynnosc != 'e':
            ...
            czynnosc = input('')
            if czynnosc == 'r':
                print('co chcesz kupić podróżniku?')
                print('1 - runa ognia 2 - runa wody 3 - runa powietrza 4 - runa ziemi 5 - runa ataku 6 - runa obrony 7 - runa zdrowia')
                ...
                if runydozobaczenia == 6:
                    print(f'Jedna runa kosztuje {cenarunyobrony} monet')
                    ilosc = int(input("Ile chcesz towaru? "))
                    if self.gracz.gold >= ilosc*cenarunyobrony:
                        self.gracz.gold -= ilosc*cenarunyobrony
                        self.gracz.runy.runadefensu += ilosc
                        print('Miło się z tobą robi interesy podróżniku \n')
                    else:
                        print('https://drive.google.com/file/d/1oGzoZKCFQO-00JSjNQu2uYWuzzVuBcvn/view?usp=sharing'*29)
                        exit()
                if runydozobaczenia == 7:
...
class bitka:
    ...
    def ruda(self):
        ...
        sleep(3)
        print(f'Ty masz {self.gracz.hp} hp i zadajesz {self.gracz.damage} hp')
        sleep(3)
        print('CO ROBISZ????!???!??!!?!?!?!?!?!?!?!?!?!?!?!?!?')
        sleep(3)
        print('a - atak, w - uciekamy, e - stoimy nic nie robimy i czekamy na śmierć')
        inp = input()
        sleep(3)
        if inp == 'a':
